...
def smooth_vector(vector, type='akima', factor=6):
    ...
    vectors = {'meanVector': meanVector, 'meanVector_scaled': meanVector_scaled,  'smoothedVector': smoothedVector, 'smoothedVector_scaled': smoothedVector_scaled,'diffVector': diffVector, 'timelabels':timelabels, 'prettyTime': prettyTime}
...
def chartMetrics(_df, title, type='akima', factor=8, days='All', metrics='All',  byStation=False,  stationGrid=False, output_file=False):
    if days == 'Weekdays':
...
def plotAllYears(_list, _title, j, akima, chartType, fig):
    ...
    for a in _list:
        ...
        plt.legend([b for b in legendItems])
        j += 1
